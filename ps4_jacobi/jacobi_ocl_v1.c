...
//	unsigned long start_time = time_ms();
	...
	cl_int err;
	cl_mem matrix_U = clCreateBuffer(context, CL_MEM_READ_ONLY, N * N * sizeof(VALUE), NULL, &err);
	cl_mem matrix_F = clCreateBuffer(context, CL_MEM_READ_ONLY, N * N * sizeof(VALUE), NULL, &err);
	cl_mem matrix_TMP = clCreateBuffer(context, CL_MEM_WRITE_ONLY, N * N * sizeof(VALUE), NULL, &err);
	...
	// add profiling information for writing F
	...
	for (int i = 0; i < IT; ++i) {
		...
		err = clEnqueueReadBuffer(command_queue, matrix_TMP, CL_TRUE, 0, N * N * sizeof(VALUE), u, 0, NULL, &ev_read_TMP);
